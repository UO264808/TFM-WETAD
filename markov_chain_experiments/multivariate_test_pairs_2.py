...
VOCAB_SIZE = (len(set(transitions_1)))**2
...
for elem in range(len(data_train_1)):
    data_train.append(data_train_1[elem] + data_train_2[elem])
...
for elem in range(len(data_test_1)):
    data_test.append(data_test_1[elem] + data_test_2[elem])
    labels_test.append(labels_test_1[elem] or labels_test_2[elem])
...
pred_table.to_csv('results_multi_two.csv', index=True)
